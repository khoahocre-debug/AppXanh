...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (form.password !== form.confirm) { toast.error('Mật khẩu không khớp'); return }
    setLoading(true)
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-xl p-7">
          <form onSubmit={handleRegister} className="space-y-4 mb-5">
            <div>
              ...
              <input type="text" required value={form.name}
                ...
                placeholder="Nguyễn Văn A" className="input h-11" autoFocus />
            </div>
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1.5">Email</label>
              <input type="email" required value={form.email}
                onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                placeholder="email@example.com" className="input h-11" />
            </div>
            <div>
              ...
              <div className="relative">
                <input type={showPass ? 'text' : 'password'} required value={form.password}
                  ...
                  placeholder="Tối thiểu 6 ký tự" className="input h-11 pr-11" minLength={6} />
            ...
            <div>
              ...
              <input type="password" required value={form.confirm}
                ...
                placeholder="Nhập lại mật khẩu" className="input h-11" />
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="text-xs font-medium" style={{ color: '#94A3B8' }}>Hoặc đăng ký với</span>
            <div className="flex-1 h-px" style={{ background: '#E2E8F0' }} />
          </div>
          ...
          <p className="text-center text-sm text-slate-500 mt-5">
            Đã có tài khoản?{' '}
            <Link href="/login" className="font-bold" style={{ color: '#2563EB' }}>Đăng nhập</Link>
          </p>
        </div>
      </div>
    </div>
