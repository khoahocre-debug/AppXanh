...
// SVG icon chuyên nghiệp thay cho emoji ✅
function SuccessIcon({ size = 40 }: { size?: number }) {
  return (
    <svg
      ...
      <path d="M22 32.5L28.8 39.3L42.5 25.5"
...
function PaymentContent() {
  ...
  // Load order + restore confirming state từ sessionStorage (tránh flicker)
  useEffect(() => {
    let active = true
    ...
    const load = async () => {
      ...
      // Restore confirming từ sessionStorage để tránh flash "pending" sau khi reload
  ...
  // Poll mỗi 5s khi đang confirming
  useEffect(() => {
    if (!polling) return
    const supabase = createClient()
    ...
    const interval = window.setInterval(async () => {
      ...
      if (data.payment_status === 'paid') {
        ...
        // router.replace thay vì window.location.href — không hard reload
        window.setTimeout(() => {
    ...
    <div className="mx-auto max-w-xl px-4 py-16 text-center">
      <div className="relative overflow-hidden rounded-[32px] border border-emerald-100 px-6 py-10 shadow-[0_24px_70px_rgba(16,185,129,0.12)]"
        ...
        <p className="mx-auto mb-8 max-w-md text-sm leading-6 text-slate-600">
          App Xanh đã ghi nhận thanh toán cho đơn{' '}
            ...
            <div className="px-6 py-6 sm:px-7">
              ...
              <div className="mt-4 rounded-2xl border border-emerald-100 bg-white/80 p-4">
                ...
                <ul className="space-y-1.5 text-sm text-slate-600">
                  ...
                  <li>• Nội dung chuyển khoản phải là{' '}
                    <span className="font-mono font-semibold text-blue-700">{content}</span>
                  </li>
