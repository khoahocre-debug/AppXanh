...
import { ThumbsUp, Reply, Star, CheckCircle2, Clock } from 'lucide-react'
...
export function ReviewsSection({ productId, productName }: Props) {
  ...
  const [form, setForm] = useState({
    name: '', rating: 0, content: '', submitting: false
  })
  ...
  const loadData = async () => {
    ...
    if (session?.user) {
      ...
      setForm(f => ({ ...f, name: profile?.full_name || session.user.email?.split('@')[0] || '' }))
  ...
  const handleLike = async (reviewId: string) => {
    ...
    if (isLiked) {
      await supabase.from('review_likes').delete()
        .eq('review_id', reviewId).eq('user_id', currentUser.id)
      await supabase.from('product_reviews')
        .update({ likes_count: Math.max(0, (reviews.find(r => r.id === reviewId)?.likes_count ?? 1) - 1) })
        .eq('id', reviewId)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.content.trim() || form.content.length < 10) {
      toast.error('Bình luận tối thiểu 10 ký tự'); return
    }
    if (hasPurchased && form.rating === 0) {
      toast.error('Vui lòng chọn số sao đánh giá'); return
    }
    ...
    const { error } = await supabase.from('product_reviews').insert({
      ...
      rating: hasPurchased && form.rating > 0 ? form.rating : null,
    ...
    <div className="space-y-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="flex-1 w-full space-y-1.5">
            {[5,4,3,2,1].map(star => {
              ...
              const pct = reviews.filter(r => r.rating).length > 0
                ? (count / reviews.filter(r => r.rating).length) * 100 : 0
              return (
      ...
      {reviews.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-slate-400 mb-1">Chưa có đánh giá nào</p>
          <p className="text-xs text-slate-300">Hãy là người đầu tiên đánh giá sản phẩm này!</p>
        </div>
      ) : (
      ...
      <div id="review-form" className="bg-white rounded-2xl border border-slate-200 p-6">
        ...
        {!replyTo && !hasPurchased && currentUser && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Star rating — chỉ cho người đã mua và không phải reply */}
          {!replyTo && hasPurchased && (
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                Đánh giá *
