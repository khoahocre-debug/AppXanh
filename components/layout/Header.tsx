...
import { ShoppingCart, Menu, X, LogOut, Package, ChevronDown, Settings, Users, Flame, Search } from 'lucide-react'
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center gap-0.5">
              {visibleLinks.map(({ href, label, hot }) => (
                <Link key={href} href={href}
                  ...
                  {hot && <Flame size={13} fill="currentColor" className="text-orange-500" />}
                  {label}
                  {hot && (
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className="hidden sm:flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition-all hover:bg-slate-50 border border-transparent hover:border-slate-200"
                style={{ color: '#94A3B8' }}
                title="Tìm kiếm (Ctrl+K)">
                <Search size={17} />
                <span className="hidden lg:inline text-xs" style={{ color: '#CBD5E1' }}>Tìm kiếm...</span>
                <span className="hidden lg:inline text-[10px] font-bold px-1.5 py-0.5 rounded-md"
                  style={{ background: '#F1F5F9', color: '#94A3B8' }}>⌘K</span>
              </button>

              {/* Search mobile button */}
              <button
                onClick={() => setSearchOpen(true)}
                className="sm:hidden p-2.5 rounded-xl text-slate-600 hover:text-blue-700 hover:bg-blue-50 transition-all">
                <Search size={20} />
              </button>
              ...
              <button onClick={openCart}
                ...
                {mounted && count > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] px-1 text-xs rounded-full flex items-center justify-center font-black"
                    style={{
                      background: '#DBEAFE',
                      color: '#1D4ED8',
